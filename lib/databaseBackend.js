...
Self.prototype.saveBlocks = function (blocks, start_height) {
  return new Promise((resolve, reject) => {
    var queries = []
    var savedBlocks = []
    ...
    for (var i = 0; i < blocks.length; i++) {
      ...
      if (blocks[i].difficulty === 0 || blocks[i].blockSize === 0 || (blocks[i].timestamp === 0 && blocks[i].index !== 0)) {
        ...
        return reject(new Error('Malformed block data detected, refusing to save batches of blocks starting from ' + start_height))
      }
    ...
    this.buildDeleteBlocksFromHeightQueries(start_height).then((deletes) => {
